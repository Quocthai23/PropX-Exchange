...
export class CommissionsService {
  ...
  async getUserStats(userId: string) {
    ...
    const totalByCurrency = rewards.reduce(
      (acc, curr) => {
  ...
  async generateClaimSignature(userId: string, dto: ClaimRewardsDto) {
    ...
    const invalidRewards = rewards.filter((r) => {
      ...
      if (r.status === 'PENDING' && r.unlockAt && r.unlockAt > new Date())
        return true;
      return false;
    });
    ...
    if (invalidRewards.length > 0) {
      throw new BadRequestException(
        'Some rewards are not available for claiming or are still locked.',
      );
    }
    ...
    if (currencies.size > 1) {
      throw new BadRequestException(
        'Cannot claim multiple currencies in a single transaction.',
      );
    }
